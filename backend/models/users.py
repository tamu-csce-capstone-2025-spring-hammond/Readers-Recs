...
def update_user(user_id, **kwargs):
    try:
        ...
        users_collection.update_one({"_id": ObjectId(user_id)}, {"$set": validated_data})
...
def update_user_settings(user_id, first_name=None, last_name=None, interests=None, demographics=None):
    try:
        existing_user = users_collection.find_one({"_id": ObjectId(user_id)})
        if not existing_user:
            return "Error: User not found."
        ...
        if interests is not None:
            update_data["interests"] = interests if isinstance(interests, list) else [interests]
        if demographics is not None:
            update_data["demographics"] = demographics if isinstance(demographics, list) else [demographics]
