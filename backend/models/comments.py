...
def is_valid_object_id(collection_name, obj_id): #TODO: put in one file and just import it
    """
...
def read_comment(comment_id):
    try:
        ...
        return CommentSchema(**comment).model_dump(by_alias=True) if comment else "Comment not found."
...
def read_comment_field(comment_id, field):
    try:
        ...
        comment = comments_collection.find_one({"_id": ObjectId(comment_id)}, {field: 1, "_id": 0})
