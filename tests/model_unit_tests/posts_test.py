...
def test_crud_post_and_fields(user_and_book):
    ...
    updated_post = {**mock_post, "title": "New", "post_text": "NewText", "tags": ["z"]}
    ...
    with patch("models.posts.is_valid_object_id", return_value=True), patch(
        "models.posts.posts_collection.insert_one",
        return_value=MagicMock(inserted_id=ObjectId(pid)),
    ), patch(
        "models.posts.posts_collection.find_one", side_effect=find_one_mock
    ), patch(
        "models.posts.posts_collection.update_one",
        return_value=MagicMock(matched_count=1),
    ), patch(
        "models.posts.posts_collection.delete_one",
        return_value=MagicMock(deleted_count=1),
    ), patch(
        "models.posts.posts_collection.find", return_value=[mock_post]
    ):
...
def test_invalid_ids_for_post():
    ...
    with patch("models.users.create_user", return_value=uid), patch(
        "models.users.delete_user"
    ), patch(
        "models.posts.is_valid_object_id", side_effect=lambda col, oid: col != "Books"
    ):
...
def test_read_post_not_found():
    ...
    with patch("models.posts.is_valid_object_id", return_value=True), patch(
        "models.posts.posts_collection.find_one", return_value=None
    ):
...
def test_get_all_posts_for_book_empty(user_and_book):
    ...
    with patch("models.posts.is_valid_object_id", return_value=True), patch(
        "models.posts.posts_collection.find", return_value=[]
    ):
...
def test_read_post_internal_exception():
    with patch("models.posts.is_valid_object_id", return_value=True), patch(
        "models.posts.posts_collection.find_one", side_effect=Exception("DB error")
    ):
...
def test_create_post_unexpected_exception():
    with patch("models.posts.is_valid_object_id", return_value=True), patch(
        "models.posts.PostSchema", side_effect=Exception("Unexpected failure")
    ):
...
def test_read_post_field_exception():
    with patch("models.posts.is_valid_object_id", return_value=True), patch(
        "models.posts.posts_collection.find_one",
        side_effect=Exception("field read error"),
    ):
...
def test_update_post_exception():
    with patch("models.posts.is_valid_object_id", return_value=True), patch(
        "models.posts.posts_collection.update_one",
        side_effect=Exception("update crash"),
    ):
...
def test_delete_post_exception():
    with patch("models.posts.is_valid_object_id", return_value=True), patch(
        "models.posts.posts_collection.delete_one", side_effect=Exception("delete boom")
...
def test_get_all_posts_for_book_exception():
    with patch("models.posts.is_valid_object_id", return_value=True), patch(
        "models.posts.posts_collection.find", side_effect=Exception("find exploded")
    ):
